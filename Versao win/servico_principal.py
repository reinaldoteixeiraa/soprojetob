...
class servico_principal(object):
    def __init__(self, dir_superv = "", FILE_LOG = "LOG_Principal.txt"):
        print("Iniciando...")
        ...
        ''' DIRETORIO BACKUP '''
        self.dir_backup = (os.getcwd() + "//Sync").replace("\\", "//")
        if not os.path.exists(self.dir_backup):
            os.makedirs(self.dir_backup)
        print("\nDiretorio backup: %s" % self.dir_backup)
        ''' DIRETORIO BACKUP '''
    ...
    def atualizar_log(self, data, versao):
        with open(self.FILE_LOG, "w") as file:
            dirpath, files = util.capturar_todos_arquivos_e_pastas(self.dir_backup)
...
def main():
    args = get_args()
    svc_p = servico_principal(args.dir_superv)
    svc_p.iniciar()
